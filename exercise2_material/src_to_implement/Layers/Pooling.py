...
class Pooling(BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        output_height = int((input_height - self.pooling_shape[1]) / self.stride_shape[0]) + 1
        output_width = int((input_width - self.pooling_shape[0]) / self.stride_shape[1]) + 1
            ...
            for j in range(num_channels):
                ...
                while current_y + self.pooling_shape[1] <= input_height:
                    ...
                    while current_x + self.pooling_shape[0] <= input_width:
                        ...
                        current_x += self.stride_shape[1]
                        output_x += 1
                    current_y += self.stride_shape[0]
                    output_y += 1
            ...
            for j in range(num_channels):
                ...
                while temp_y + self.pooling_shape[1] <= input_height:
                    ...
                    while temp_x + self.pooling_shape[0] <= input_width:
                        ...
                        temp_x += self.stride_shape[1]
                        output_x += 1
                    temp_y += self.stride_shape[0]
                    output_y += 1
